...
- **M3 Extended** (Light, Dark) — `success`, `onSuccess`, `successContainer`, `onSuccessContainer`. **Added by us**, not from the original Figma export, because dashboards need green up / red down. Kept separate so `M3 Color` stays a mirror of the Figma export.
...
| `Chip` | `type?` (assist\|filter\|input\|suggestion), `selected?`, `leadingIcon?`, `onRemove?`, children=label | Selected → `secondaryContainer` fill + built-in check icon; `input` shows a trailing ×. |
