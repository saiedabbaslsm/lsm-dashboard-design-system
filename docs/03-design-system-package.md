...
| `Badge` | `tone?` (neutral\|danger\|warning\|success\|info), `dot?`=true, children=label, + native span attrs | **Inert** status label (RAG, states). Deliberately borderless — a border reads as clickable. Soft tinted fill + saturated dot + dark AA-contrast text; per-tone `--ds-badge-dot`/`--ds-badge-fg` verified ≥4.5:1 in both themes. `danger` takes its text from `error` because `onErrorContainer` is `#111` in the Figma export. |
